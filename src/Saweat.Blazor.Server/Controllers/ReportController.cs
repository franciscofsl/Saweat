...
public partial class ReportController : Controller
{
    ...
    {
        var urlToReplace = string.Format(format: "{0}://{1}{2}/{3}/", Request.Scheme, Request.Host.Value, Request.PathBase, "ssrsproxy");
        var requestedUrl = Request.GetDisplayUrl().Replace(urlToReplace, newValue: "", StringComparison.InvariantCultureIgnoreCase);
        var reportServerIndex = requestedUrl.IndexOf(value: "/ReportServer", StringComparison.InvariantCultureIgnoreCase);
        if (reportServerIndex == -1) reportServerIndex = requestedUrl.IndexOf(value: "/Reports", StringComparison.InvariantCultureIgnoreCase);
        var reportUrlParts = requestedUrl.Substring(0, reportServerIndex).Split('/');

        var url = string.Format(format: "{0}://{1}:{2}{3}", reportUrlParts[0], reportUrlParts[1], reportUrlParts[2],
        ...
        using (var httpClient = CreateHttpClient())
        {
            ...
            {
                if (responseMessage.Content.Headers.ContentType != null && responseMessage.Content.Headers.ContentType.MediaType == "text/html") await WriteResponse(Request, url, responseMessage, Response, false);
                else
    ...
    private async Task<HttpResponseMessage> ForwardRequest(HttpClient httpClient, HttpRequest currentReqest, string url)
    {
        ...
        foreach (var header in currentReqest.Headers)
            if (header.Key != "Host")
                proxyRequestMessage.Headers.TryAddWithoutValidation(header.Key, new string[]
                {
                    header.Value
                });
        ...
        if (currentReqest.Method == "POST")
            using (var stream = new MemoryStream())
            {
                ...
                if (body.IndexOf("AjaxScriptManager") != -1)
                {
                    ...
                    proxyRequestMessage.Content.Headers.Add(name: "Content-Type", new string[]
                    {
                        currentReqest.ContentType
                    });
                }
            }
    ...
    private static async Task WriteResponse(HttpRequest currentReqest, string url, HttpResponseMessage responseMessage, HttpResponse response, bool isAjax)
    {
        ...
        var ReportServer = url.Contains(value: "/ReportServer/", StringComparison.InvariantCultureIgnoreCase) ? "ReportServer" : "Reports";

        var reportUri = new Uri(url);
        var proxyUrl = string.Format(format: "{0}://{1}{2}/ssrsproxy/{3}/{4}/{5}", currentReqest.Scheme, currentReqest.Host.Value, currentReqest.PathBase,
        ...
        if (isAjax && result.IndexOf("|") != -1)
        {
            ...
            {
                delimiterIndex = result.IndexOf(value: "|", index);
                ...
                if (content.Contains(value: "./ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase)) content = content.Replace(oldValue: "./ReportViewer.aspx", $"{proxyUrl}/{ReportServer}/Pages/ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase);
                else content = content.Replace(oldValue: "ReportViewer.aspx", $"{proxyUrl}/{ReportServer}/Pages/ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase);

                builder.Append(string.Format(format: "{0}|{1}|{2}|{3}|", content.Length, type, id, content));
            }
        ...
        else
        {
            ...
            if (result.Contains(value: "./ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase)) result = result.Replace(oldValue: "./ReportViewer.aspx", $"{proxyUrl}/{ReportServer}/Pages/ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase);
            else result = result.Replace(oldValue: "ReportViewer.aspx", $"{proxyUrl}/{ReportServer}/Pages/ReportViewer.aspx", StringComparison.InvariantCultureIgnoreCase);
        }

        response.Headers.Remove("Content-Length");
        response.Headers.Add(key: "Content-Length", new string[]
        {
